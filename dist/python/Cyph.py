...
class Cyph:
    ...
    @staticmethod
    def generateLink(options = None,services = None):
        if (options is None):
            options = []
        if (services is None):
            services = Cyph.services
        id = Cyph.generateGuid(7)
        return _hx_AnonObject({'id': id, 'link': (((((HxOverrides.stringOrNull(((Cyph.services.video if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.video,None) > -1)) else (Cyph.services.voice if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.voice,None) > -1)) else Cyph.services.chat)))) + HxOverrides.stringOrNull((("&" if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.modestBranding,None) > -1)) else "")))) + HxOverrides.stringOrNull((("$" if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.disableP2P,None) > -1)) else "")))) + HxOverrides.stringOrNull((("%" if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.nativeCrypto,None) > -1)) else "")))) + ("null" if id is None else id)) + HxOverrides.stringOrNull(Cyph.generateGuid(19)))})
...
Cyph.services = _hx_AnonObject({'backend': "https://api.cyph.com", 'chat': "https://cyph.im/#", 'video': "https://cyph.video/#", 'voice': "https://cyph.audio/#"})
Cyph.options = _hx_AnonObject({'disableP2P': 1, 'modestBranding': 2, 'nativeCrypto': 3, 'telehealth': 4, 'video': 5, 'voice': 6})
