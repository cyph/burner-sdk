...
class Cyph:
    ...
    @staticmethod
    def generateLink(options,services):
        id = Cyph.generateGuid(7)
        return _hx_AnonObject({'id': id, 'link': ((((HxOverrides.stringOrNull(((services.telehealth if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.telehealth,None) > -1)) else (services.video if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.video,None) > -1)) else (services.voice if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.voice,None) > -1)) else services.chat))))) + HxOverrides.stringOrNull((("!" if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.modestBranding,None) > -1)) else "")))) + HxOverrides.stringOrNull((("$" if ((python_internal_ArrayImpl.indexOf(options,Cyph.options.disableP2P,None) > -1)) else "")))) + ("null" if id is None else id)) + HxOverrides.stringOrNull(Cyph.generateGuid(19)))})
    ...
    def initiateSession(apiKey,options = None,services = None,onData = None,onError = None):
        ...
        if (Reflect.field(services,"backend") is None):
            Reflect.setField(services,"backend",Cyph.services.backend)
        if (Reflect.field(services,"telehealth") is None):
            Reflect.setField(services,"telehealth",(HxOverrides.stringOrNull(services.chat) + "video/"))
        if (Reflect.field(services,"video") is None):
            Reflect.setField(services,"video",(HxOverrides.stringOrNull(services.chat) + "video/"))
        if (Reflect.field(services,"voice") is None):
            Reflect.setField(services,"voice",(HxOverrides.stringOrNull(services.chat) + "audio/"))
        linkData = Cyph.generateLink(options,services)
        def _hx_local_0(data):
            onData(linkData.link)
        Cyph.request(((HxOverrides.stringOrNull(Reflect.field(services,"backend")) + "/preauth/") + HxOverrides.stringOrNull(linkData.id)),True,[_hx_AnonObject({'k': "Authorization", 'v': apiKey})],[],_hx_local_0,onError)
...
class SecureRandom:
    ...
    def getSecureRandomBytes(length):
        ...
        try:
            ...
            _hx_bytes = RandomOs.urandom(length)
            _g1 = 0
            _g = length
            while (_g1 < _g):
                n = _g1
                _g1 = (_g1 + 1)
                out.b[n] = ((_hx_bytes[n] if n >= 0 and n < len(_hx_bytes) else None) & 255)
            return out
        except Exception as _hx_e:
            _hx_e1 = _hx_e.val if isinstance(_hx_e, _HxException) else _hx_e
            e = _hx_e1
...
class haxe_Http:
    ...
    _hx_methods = ["setHeader", "setParameter", "setPostData", "request", "customRequest", "readHttpResponse", "readChunk", "onData", "onError", "onStatus"]
    _hx_statics = ["PROXY"]
...
Cyph.services = _hx_AnonObject({'backend': "https://api.cyph.com", 'chat': "https://cyph.im/#", 'telehealth': "https://video.cyph.healthcare/#", 'video': "https://cyph.video/#", 'voice': "https://cyph.audio/#"})
Cyph.options = _hx_AnonObject({'disableP2P': 1, 'modestBranding': 2, 'telehealth': 3, 'video': 4, 'voice': 5})
